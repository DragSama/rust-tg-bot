...
#[must_use = "CreateNewStickerSet does nothing until you `send` it"]
#[derive(Serialize)]
...
impl<'a> CreateNewStickerSet<'a> {
    pub fn new(bot: &'a Bot, user_id: i32, name: String, title: String, emojis: String) -> Self {
        Self {
            user_id: user_id,
            name: name,
            title: title,
    ...
    pub async fn send(self) -> Result<bool> {
        ...
        let resp = self.bot.send("createNewStickerSet", Some(string)).await?;
